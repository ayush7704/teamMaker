import React, { useContext, useRef, useMemo, useState, useEffect, lazy, Suspense, memo } from 'react'
...
const story = [
  {
    title: "Inspiration",
    lines: [
      "Our daily football games always began with arguments over team selection.",
      "Even in cricket, every time we gathered the real trouble was who would pick the teams. The one who did had to handle debates, jokes, complaints, and the captain-type pressure till the match ended. No one liked doing that.",
  ...
  },
  {
    title: "Background",
    lines: [
      "With just one React practice project before, I was excited for a bigger challenge.",
    ]
  },
  {
    ...
    lines: [
      ...
      "As my second React project, it pushed me to learn more and improve."
    ]
  },
  {
    title: "Key Features",
    lines: [
      { line: "Fair & Randomized Team Selection – No more debates!", },
      { line: "Save unlimited projects locally in the browser.", },
      { line: "Easy-to-Use – Simple and clear for everyone.", },
      { line: "Developer Growth – A big milestone in my coding journey so far.", }
    ]
  },
  {
    title: "Outcome",
...
function AboutUs() {
  ...
  return (
    <section className='pb-[6rem]'>
      ...
      {/* // lg:min-h-[80vh] */}
      <div className=" md:min-h-[50vh] grid sm:grid-cols-[repeat(auto-fit,minmax(18.75rem,1fr))] items-center justify-items-center gap-x-5 gap-y-3 p-[6vw_3vw] mb-[7vh]">
        {/* cube start  */}
        <div className='w-full grid place-items-center lg:min-h-[250px] min-h-[9.375rem] pt-20 pb-16 text-[0.9rem]'>
          <div onClick={(e) => { e.stopPropagation() }} ref={cubeRef} className="gridCube relative lg:w-[11.25em] lg:h-[11.25em] md:w-[9.375em] md:h-[9.375em] w-[6.25em] h-[6.25em]">
            {Array(6).fill(null).map((_, val) => (
              ...
              ${val + 1 === 1 ? `lg:[transform:translateZ(5.625em)_rotateY(360deg)] md:[transform:translateZ(4.6875em)_rotateY(360deg)] [transform:translateZ(3.125em)_rotateY(360deg)]` : ""} ${val + 1 === 2 ? "[transform:rotateY(-270deg)_translateX(50%)] origin-[top_right]" : ""} ${val + 1 === 3 ? "[transform:rotateY(270deg)_translateX(-50%)] origin-[center_left]" : ""} ${val + 1 === 4 ? "[transform:rotateX(90deg)_translateY(-50%)] origin-[top_center]" : ""} ${val + 1 === 5 ? "[transform:rotateX(-90deg)_translateY(50%)] origin-[bottom_center]" : ""} ${val + 1 === 6 ? `lg:[transform:translateZ(-5.625em)_rotateY(180deg)] md:[transform:translateZ(-4.6875em)_rotateY(180deg)] [transform:translateZ(-3.125em)_rotateY(180deg)]` : ""}`}>
                {
        ...
        {/* cube ends  */}
        {/* text starts  */}
        <div className='flex sm:order-[-1] flex-col text-[clamp(1.25rem,5vw,2.25rem)] sm:text-[4vw] lg:max-2xl:text-[clamp(3rem,4vw,5.1rem)] 2xl:text-[clamp(4rem,4vw,6.5rem)] Bricolage font-medium'>
      ...
      <div className='Bricolage font-medium grid grid-cols-[100%] p-[3vw]'>
        <div className='flex flex-col text-[clamp(1.25rem,5vw,2.25rem)] sm:text-[4vw] lg:max-2xl:text-[clamp(3rem,4vw,5.1rem)] 2xl:text-[clamp(4rem,3.8vw,6.5rem)]'>
          <div className='max-lg:text-center leading-[noraml]'><span>Design & Developed by </span>
            <div className='relative inline-block text-[--theme]' href="https://ayushnagar.netlify.app" target='_blank'><div> Ayush Nagar
              <div ref={ayushLineThrough} className='absolute w-full sm:h-[5px] h-[3px] bg-[#c81417] top-1/2'></div></div>
            </div>
          </div>
      ...
      <div className='flex flex-wrap gap-8 p-[3vw] max-sm:text-[1rem] sm:text-[1.1rem] md:text-[1.2rem]'>
        <div className='max-lg:flex-[1_1_100%] lg:sticky lg:top-[3vh] lg:self-baseline grid place-items-center'>
          <img onLoad={() => setimgloaded(true)} src={ayushimage} alt="lazy photo of developer" className='ayush 2xl:max-w-[32rem] lg:max-w-[26rem] sm:max-w-[20rem] max-w-[19rem] aspect-square drop-shadow-[5px_7px_7px_black]' />
        </div>
        <div className='flex-1 max-lg:flex-[1_1_100%] px-2 max-w-[40rem]'>
          <h2 className="text-[1.05em] font-extralight mb-4">The Story Behind Random Team Maker</h2>
          {
            story.map((section, index) => {
              if (section.title == "Key Features") {
                return (
                  <section key={index} className="mb-6 2xl:text-[1.1em]">
                    <h2 className="text-[0.98em] mb-2 font-medium Bricolage storyHeading">{section.title}</h2>
                    <ul className="text-[0.90em] list-none pr-6 space-y-1 grid gap-1">
                      {section.lines.map((line, index) => {
                        return (
                          <li key={index} className={`storyLines will-change-[opacity,transform] flex items-baseline gap-2`}>
                            <span>
                ...
                return (<section key={index} className="mb-6 2xl:text-[1.1em]">
                  <h2 className="text-[0.98em] mb-2 font-medium Bricolage storyHeading">{section.title}</h2>
                  <ul className="text-[0.90em] list-disc px-5 pl-6 space-y-1 grid gap-1">
                    {section.lines.map((line, index) => {
                      return (
                        <li key={index} className='storyLines will-change-[opacity,transform]'>{line}</li>
                      )
                    }
                    )}
                  </ul>
