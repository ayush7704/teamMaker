...
function AboutUs() {
  ...
  return (
    <section className='pb-[6rem]'>
      ...
      <div className="lg:min-h-[80vh] md:min-h-[50vh] grid sm:grid-cols-[repeat(auto-fit,minmax(18.75rem,1fr))] items-center justify-items-center gap-x-5 gap-y-3 p-[6vw_3vw] mb-[7vh]">
        {/* cube start  */}
        ...
        {/* cube ends  */}
        {/* text starts  */}
        <div className='flex sm:order-[-1] flex-col text-[7vw] sm:text-[6vh] md:text-[7vh] lg:text-[8vh] xl:text-[9vh] Bricolage font-medium'>
      ...
      <div className='Bricolage font-medium grid grid-cols-[100%] p-[3vw]'>
        <div className='flex flex-col text-[7vw] sm:text-[6vh] md:text-[7vh] lg:text-[8vh] xl:text-[9vh]'>
          <div className='max-lg:text-center leading-[noraml]'><span>Design & Developed by </span>
            <div className='relative inline-block text-[--theme]' href="https://ayushnagar.netlify.app" target='_blank'><div> Ayush Nagar
              <div ref={ayushLineThrough} className='absolute w-full sm:h-[5px] h-[4px] bg-[#c81417] top-1/2'></div></div>
            </div>
          </div>         
      ...
      <div className='flex flex-wrap gap-8 p-[3vw] max-sm:text-[1rem] sm:text-[1.1rem] md:text-[1.2rem]'>
        <div className='max-lg:flex-[1_1_100%] lg:sticky lg:top-[3vh] lg:self-baseline grid place-items-center'>
          <img onLoad={() => setimgloaded(true)} src={ayushimage} alt="lazy photo of developer" className='ayush 2xl:max-w-96 lg:max-w-96 sm:max-w-72 max-w-56 aspect-square drop-shadow-[5px_7px_7px_black]' />
        </div>
          ...
          <h2 className="text-[1.05em] font-extralight mb-4">The Story Behind Random Team Maker</h2>
          {
            story.map((section, index) => (
              <section key={index} className="mb-6">
                ...
                <ul className="text-[0.90em] list-disc px-5 pl-6 space-y-1">
                  {section.lines.map((line, index) => {
                  return  (typeof line === 'object' ?                      
                      <li key={index} className={`storyLines checked_list [list-style-image:url('/check.svg')] marker:text-[2em] marker:leading-[0]`}>
                       {line?.line}</li>:
