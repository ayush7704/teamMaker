...
import { Draggable } from "gsap/all"; // No InertiaPlugin needed!
...
function aboutUs() {
  ...
  return (
    <>
      ...
      {/* <div className='relative grid justify-center mt-10 gap-5'>
      ...
      <div ref={dragAreaRef} className="min-h-[80vh] grid sm:grid-cols-[repeat(auto-fit,minmax(18.75rem,1fr))] items-center justify-items-center gap-5">
        <div className='w-full grid place-items-center lg:h-[250px] h-[150px]'>
          <div onClick={(e) => { e.stopPropagation() }} ref={cubeRef} className="gridCube relative lg:w-[200px] lg:h-[200px] w-[100px] h-[100px]">
            {Array(6).fill(null).map((_, val) => (
              <div key={val} onClick={(e) => { e.stopPropagation() }} className={`gridChilds gridChild${val + 1} absolute h-full w-full grid grid-cols-3 grid-rows-3 rounded-sm overflow-hidden bg-[#000000] ${val + 1 === 1 ? "lg:[transform:translateZ(100px)_rotateY(360deg)] [transform:translateZ(50px)_rotateY(360deg)]" : ""} ${val + 1 === 2 ? "[transform:rotateY(-270deg)_translateX(50%)] origin-[top_right]" : ""} ${val + 1 === 3 ? "[transform:rotateY(270deg)_translateX(-50%);] origin-[center_left]" : ""} ${val + 1 === 4 ? "[transform:rotateX(90deg)_translateY(-50%)] origin-[top_center]" : ""} ${val + 1 === 5 ? "[transform:rotateX(-90deg)_translateY(50%)] origin-[bottom_center]" : ""} ${val + 1 === 6 ? "lg:[transform:translateZ(-100px)_rotateY(180deg)] [transform:translateZ(-50px)_rotateY(180deg)]" : ""}`}>
                {
        ...
        <div className='flex flex-col text-[10vw] sm:text-[10vh] p-[3vw] saira-stencil'>
      ...
      {/* <div className='h-screen w-full grid place-items-center'>
        <div id='gridCube' className="gridCube relative lg:w-[300px] lg:h-[300px] w-[150px] h-[150px]">
          {Array(6).fill(null).map((_, val) => (
            <div key={val} className={`gridChilds gridChild${val + 1} absolute h-full w-full grid grid-cols-3 grid-rows-3 rounded-md overflow-hidden bg-[#000000] ${val+1 === 1 ? `lg:[transform:translateZ(${(document.querySelector('.gridCube').getBoundingClientRect().height/2)}px)_rotateY(180deg)]`:""} ${val+1 === 2 ?"[transform:rotateY(-270deg)_translateX(50%)] origin-[top_right]":""} ${val+1 === 3 ?"[transform:rotateY(270deg)_translateX(-50%);] origin-[center_left]":""} ${val+1 === 4 ?"[transform:rotateX(90deg)_translateY(-50%)] origin-[top_center]":""} ${val+1 === 5 ?"[transform:rotateX(-90deg)_translateY(50%)] origin-[bottom_center]":""} ${val+1 === 6 ?`lg:[transform:translateZ(${(document.querySelector('.gridCube').getBoundingClientRect().height/2)}px)]`:""}`}>
              {Array(12).fill(null).map((_, val) => (
                <div key={val} className='border hover:bg-[#141414] transition duration-200 ease-in-out border-[#FFFFFF]'></div>
              ))}
            </div>
          ))}          
        </div>
